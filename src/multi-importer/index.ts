...
import { getDescriptionsDownloadUrl, getFacilityDownloadUrl, getHotelContentDownloadUrl, getImagesDownloadUrl } from '../client';
...
export const importer = async (
  ...
  await Promise.all(Object.keys(indexTypes).map(async (indexType: string) => {
    ...
    const file = indexTypes[indexType].file;
